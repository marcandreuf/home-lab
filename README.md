...
- morning-terminals.sh – opens the morning workspace as positioned terminal windows; takes the project directory to open Claude Code in, e.g. `./morning-terminals.sh foo` for `~/projects/foo`, or no argument for `~/projects` itself (X11 only; run with --install on a new machine)
