...
Renamed because it is no longer git-only. Upstream blocks `git push`, `reset --hard`, `clean -f`, `branch -D`, `checkout .` and `restore .`; this copy keeps all of those except plain `git push` — see below — and adds history rewrites (`filter-branch`, `reflog expire`, `gc --prune`, `update-ref -d`), `gh repo delete/archive/rename`, `gh api` with any write flag, `rm -rf` on absolute paths and `$HOME`, and `docker prune` / `volume rm` / `compose down -v`. The case that prompted the widening — deleting a repo through `gh api -X DELETE` — is absent upstream, and it is the one that cannot be undone.

**Plain `git push` is downgraded from blocked to confirmed.** Upstream refuses it outright. Pushing is ordinary development work here, so refusing it means editing a script to do a daily task, and a guardrail people route around stops being one. It is added to `permissions.ask` instead, which asks every time and — unlike simply leaving it off the allow list — cannot be turned into an allow rule by answering "don't ask again" in the moment. The force variants stay blocked outright, since they overwrite remote history.
